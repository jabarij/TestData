...
namespace TestData.Building.Dynamic
{
    partial class DynamicBuilderTests
    {
        ...
        {
            public class ReferenceType : Overwrite
            {
                [Theory]
            ...
            public class ValueType : Overwrite
            {
                [Theory]
                [InlineData(nameof(TestStruct.PublicGetPublicSet), 1)]
                [InlineData(nameof(TestStruct.PublicGetInternalSet), 1)]
                [InlineData(nameof(TestStruct.PublicGetPrivateSet), 1)]
                [InlineData(nameof(TestStruct.PublicReadOnlyAutoProperty), 1)]
                [InlineData(nameof(TestStruct.PublicGetOfPrivateField), 1)]
                [InlineData(nameof(TestStruct.InternalGetInternalSet), 1)]
                [InlineData(nameof(TestStruct.InternalGetPrivateSet), 1)]
                public void ShouldCallPropertySetterForGivenProperty(string propertyName, object expectedValue)
                {
                    // arrange
                    var expectedInstance = new TestStruct();
                    var instanceFactoryMock = new Mock<IInstanceFactory<TestStruct>>();
                    instanceFactoryMock
                        .Setup(e => e.Create(It.IsAny<IEnumerable<INamedPropertyOverwriter>>()))
                        .Returns(expectedInstance);
                    var propertySetterMock = new Mock<IPropertySetter>();

                    var sut = new DynamicBuilder<TestStruct>(
                        instanceFactory: instanceFactoryMock.Object,
                        propertySetter: propertySetterMock.Object);
                    var expectedProperty = expectedInstance.GetType().GetProperty(propertyName, BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic);
